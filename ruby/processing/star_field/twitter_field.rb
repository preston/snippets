...
class TwitterField < Processing::App
  ...
  def setup
    ...
    # puts library_loaded?('twitter-0.6.8')
    frame_rate FRAME_RATE
    

    # config = YAML.load(File.read(File.join(ENV['HOME'], '.twitter', 'config')))
    # puts config
    # Twitter::Client.configure do |conf|
    #    conf.protocol = :http
    #    conf.port = 80 # 443 for ssl
    #  end
    # @twitter = Twitter::Client.new(config['default'])
    # @twitter = Twitter::Base.new(config['default'])
...
TwitterField.new :width => 1000, :height => 800, :title => "Twitter status messages flying through space."
